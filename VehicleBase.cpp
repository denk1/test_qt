...
VehicleBase::VehicleBase(const VehiclePrototype& prototype)
	...
	, mSpeedToKmh(0.5f * 3.6f) // 0.5 - из пикселей в метры, 3.6 - из м/c в км/ч
	, mSpeedToMph(0.5f * 2.236936f) // 0.5 - из пикселей в метры, 2.236936 - из м/c в м/ч
{
...
void VehicleBase::create(Ogre::SceneNode* parentSN)
{
	...
	for(int i = 0; i < 4; ++i)
	{
		// Создаем геометрию и помещаем её на сцену
	...
	// Создаем SceneNode для креплений
...
void VehicleBase::updateVehicle(const Ogre::Real dt)
{
	...
	{
		// Пропускаем слишком быстрый кадр
		mSkipedTime += dt;
		return;
	}

	// Проверяем сонаправленность направления и движения
...
void VehicleBase::updateVehicleSteering(const Ogre::Real dt)
{
	...
	if ((mSteeringLeft == mSteeringRight) && ( mSteeringRight == false))
			// Руль отпущен и возвращается в исходное положение
	...
	else
	{
		...
		if (mSteeringLeft)
		{
			// Влево
			if (mSteering < 0)
				mSteering +=  mProperties.mSteeringIncrement * 4.f * dt * speedAffectionIncrement; // Если колесо повернуто в другую сторону, то поворот ускоряется
			else
